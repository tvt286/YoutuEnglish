...
import youtuvideos.tranty.vn.youtuvideos.interfaces.ItemModuleListeners;
...
public class LearningActivity extends BaseActivity implements YouTubePlayer.OnInitializedListener {
    ...
    private boolean isPaused = false, isPlay = false, isRequest = false;
    ...
    private int knowledge_user_id;
    private boolean isUpdate = false, isFromNoti = true;
    ...
    private void getData() {
        Intent i = getIntent();
        knowledge_user_id = i.getIntExtra("knowledge_user_id", 0);
        if (knowledge_user_id == 0) {
            isFromNoti = false;
            knowledgeUser = (KnowledgesUserVO) getIntent().getSerializableExtra("KnowledgeUser");
            knowledge_user_id = knowledgeUser.id;
            setData(knowledge_user_id);
        } else {
            isFromNoti = true;
            setData(knowledge_user_id);
        }
        ...
        UsersRequest.getKnowledges(id, new AbstractResponse() {
            ...
            public void onSuccess(int error_code, String message, Object obj) {
                ...
                if (knowledgeUser != null) {
                    ...
                    // set data
                    tvTitle.setText(moduleSelected.title);
                    tvLength.setText(Util.secondsToString(moduleSelected.length));
                    adapter.setArrModules(arrModules);
                    youTubePlayerFragment.initialize(Constants.KEY.YOUTUB_API_KEY, LearningActivity.this);
                }
            }
        });
    }
    ...
    public void onInitializationSuccess(YouTubePlayer.Provider provider, final YouTubePlayer youTubePlayer, boolean b) {
        if (!b) {
            ...
            recyclerView.post(new Runnable() {
                @Override
                public void run() {
                    recyclerView.smoothScrollToPosition(videoSelected);
                }
            });
        }
    }
    ...
    private void isCommpleted() {
        if (timer != null)
            timer.cancel();
        if (isRequest) {
            int time = Math.min(7 * 60, (moduleSelected.length * 2) / 3);
            if (timeCount > 5) {
                Log.d("video", "video completed");
                isUpdate = true;
                arrModules.get(videoSelected).completed = 1;
                ModulesRequest.completeModuleUser(knowledgeUser.id, moduleSelected.id, new AbstractResponse() {
                    @Override
                    public void onSuccess(int error_code, String message, Object obj) {
                        super.onSuccess(error_code, message, obj);
                        adapter.notifyItemChanged(videoSelected, arrModules.get(videoSelected));
                    }
    ...
    private void finishActivity(){
        ...
        intent.putExtra("knowledgeUser", (Serializable) knowledgeUser);
        intent.putExtra("isUpdate",isUpdate);
        intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
        setResult(Constants.ACTIVITY.ACTIVITY_PLAYVIDEO_SUCCESS,intent);
        finish();
    }
